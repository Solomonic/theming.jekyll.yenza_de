...
If we want to add the author property like tested in the condition above, we just update it to:
{% highlight yaml %}
---
layout: post
title: faster, cheaper, safer
...
The test and use of the new property in the layout is fairly simple
{% highlight html %}
{% raw %}{% if post.author %}{% endraw %}
...
##### Content management
Let's assume we have a group of people responsible for the content of the website. We call them Editors and we expect a low technical understanding. Their job is actually to create content and therefor to give live and blood to FantasticContent.Online. Visitor will be attracted by their work and not how lean the website is done or if it loads 0.2s faster or slower. We want to make the Editor job as easy as possible, so they can focus on the important things as creating fantastic content. 

Editors will require training to use any system, so instead of teaching the correct use of the CMS, it should be possible to teach them how to create markdown files and submit them into a prepared release circle. There are markdown readers for any OS available and even [good online tools](http://dillinger.io/ 'Online markdown editor and reader ...') which will support the content-team with the creation and update of the files. I suggest that the understanding and ability to create those markdown files is more rapid and connected with less troubles then handling content creation in any CMS. Most editors prefer to write offline and anyway, so to ask for a well-formated file is a very reasonable. 

This approach can be very valuable especially in areas with slow and un-reliable internet connections, since the content itself can be created offline and submitted by email, ftp or github as soon as the connection is available again, saving a lot of band-width in the process compared with the usual through-the-web creation of a traditional CMS.

### SEO and marketing

I just work for a very short time with static site generators, so I'm not able to back up the following assumption with real world data, but nevertheless are we able to make some qualified guesses what we can expect for **S**earch **E**ngine **O**ptimization or in better word the indexing behavior. I don't know exactly how a search engine indexes but could identify setups which work very well and helped several of my customer to get the top-spot for the desired keywords. It never hurts, if
...
This blog post have the following Frontmatter variables set:
...
You also can change the general URL mapping for all post or set static *permalink*s for special posts, we have every freedom we need, always just one Frontmatter variable away. To generate a sitemap is no problem and every other identified good SEO behavior seems very possible. Once setup I suggest an excellent indexing. I will start to collect data over the next weeks and include it in the follow up SEO articles but my expectation are high. 

An popular approach to gather data about the vistor preferences and improve towards better [user experience (UX)](http://www.forbes.com/sites/forbestechcouncil/2015/11/19/good-ux-is-good-business-how-to-reap-its-benefits/ 'Importance of UX on Forbes.com...') is split-testing, or **A/B** campaigns. Usually is the same content presented with 2 different layouts, to see which is better received and lead to higher convertion rates. With the pre-compiled websites it is very easy to implement those within one domain, just generate the same website 2 times with the different layouts, upload both sites in folder A and folder B and set your splitter algrorithm in front to do the splitting. 
...
Besides the free hosting options with certain hosters, do we gain a new freedom when it comes to content. It is in any case way easier to move to newer and faster servers if you just need to move our assets as images and a bunch of html files. Hosting providers often offer very cheap services for the first year and increase the prices in the second and following years. This strategy works well because customers go for the low price first, while all their work in the CMS will be really hard to move to another location. This migration usually costs way more then a year of hosting, so they got you don't they?
Nope, backup your images and upload them with the latest generated version of your project to the new server, done. Neiter Layout nor Content are chained to the hoster anymore and you want them backed up anyway at any time.

Content data is now stored in well-formated text-files in a accessable central location instead of a local database. This is **very** useful for any kind of automated content re-use as in RSS, MailChimp Campaigns or new and more exciting possibilities.

You don't really need to worry about security aspects of your project, a static page can not be hacked in the usual sense. A hacker could interfere with your ftp/ sftp connection but it is unlikely and more important you don't need to worry that the CMS itself allows exploits. 

### Summary

As a professional I see the approach of a pre-compiled CMS as superior to certain server-side applications. To be able to deploy different layout versions of the same content very cheap and very fast opens up a big field of scientific and business opportunities. My very personal prognosis in November 2015 is that variations of pre-compiled CMS will take a huge market share as soon there is a free Windows GUI or Android App deploying automated lean websites into a free cloud hosting. Many server-side CMS **just** show content the customer actually don't want to touch. This seems actually like a lot of trouble, since hosting, setup and maintance are way more expensive and the content is stuck in one database, mostly pretty worthless without the CMS itself.

The biggest drawback of this approach is clearly the missing real-time interaction. Nevertheless is it very easy to include any kind of widgets for Ajax embeddings or similar, so 3rd party services can be utilized to overcome this disadvatages. A good example for starters is to use the facebook comments and likes to gain feedback, include a google form for an interactive site-search and link to a MailChimp as newletter system and basic CRM. More details in the follow up for the strategic setup of FantasticContent.Online, including amazing other tools we have available for free in our days. 
